...
def generate_report(analysis_result: dict) -> str:
    ...
    total_expense_str = f"-{analysis_result['total_expense']:.2f}" if analysis_result['total_expense'] > 0 else f"{analysis_result['total_expense']:.2f}"
    report_lines.append(f"总支出: {total_expense_str}")
