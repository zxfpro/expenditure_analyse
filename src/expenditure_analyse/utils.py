...
def get_date_range_from_query(query: str) -> tuple[datetime, datetime]:
    ...
    # Get current datetime inside the function to respect mocking
    ...
    if "今天" in query_lower:
        ...
        start_of_this_week = current_datetime - timedelta(days=current_datetime.weekday())
        ...
        end_date = current_datetime + timedelta(days=(6 - current_datetime.weekday()))
        start_date = start_date.replace(hour=0, minute=0, second=0, microsecond=0)
        end_date = end_date.replace(hour=23, minute=59, second=59, microsecond=999999)
    elif "上月" in query_lower or "过去一月" in query_lower:
        # Last month
        first_day_of_this_month = current_datetime.replace(day=1)
        ...
        next_month = current_datetime.replace(day=28) + timedelta(days=4)  # Go to the 4th day of next month
...
def extract_date_from_query(query: str) -> datetime | None:
    ...
    if match:
        # Get current datetime inside the function to respect mocking
        current_datetime = datetime.now()
        return datetime(current_datetime.year, int(match.group(1)), int(match.group(2)))
